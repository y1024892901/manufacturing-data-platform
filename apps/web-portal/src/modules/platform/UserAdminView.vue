...
async function load(){loading.value=true;try{const[u,r,d]=await Promise.all([http.get('/admin/users',{params:{keyword:keyword.value||undefined,page:page.value,size:size.value}}),http.get('/admin/role-options'),http.get('/admin/department-options')]);users.value=u.data.data.content;total.value=u.data.data.totalElements;roles.value=r.data.data;departments.value=d.data.data}finally{loading.value=false}}
...
async function save(){if(!form.username||!form.realName)return ElMessage.warning('请填写用户名和姓名');saving.value=true;try{const response=editingId.value?await http.put(`/admin/users/${editingId.value}`,form):await http.post('/admin/users',form);const id=editingId.value||response.data.data.id;await http.put(`/admin/users/${id}/data-scope`,{scopeType:form.dataScopeType,values:[]});ElMessage.success(editingId.value?'用户已更新':'用户已创建');dialog.value=false;await load()}catch(e:any){ElMessage.error(e.message||'保存失败')}finally{saving.value=false}}
...
async function importUsers(event:Event){const input=event.target as HTMLInputElement;const file=input.files?.[0];if(!file)return;const data=new FormData();data.append('file',file);try{const response=await http.post('/admin/users/import',data);ElMessage.success(`导入完成：新增 ${response.data.data.created}，跳过 ${response.data.data.skipped}`);await load()}catch(e:any){ElMessage.error(e.message||'导入失败')}finally{input.value=''}}
onMounted(load)
</script>
<template>
