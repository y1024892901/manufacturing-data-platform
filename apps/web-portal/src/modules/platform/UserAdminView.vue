...
function create(){editingId.value=null;Object.assign(form,{username:'',initialPassword:'Test@123456',realName:'',empCode:'',deptCode:'',positionName:'',email:'',mobile:'',demoAccount:true,roleIds:[],systems:[],dataScopeType:'ROLE',dataScopeValue:''});dialog.value=true}
function edit(row:any){editingId.value=row.id;Object.assign(form,{username:row.username,initialPassword:'',realName:row.realName,empCode:row.empCode||'',deptCode:row.deptCode||'',positionName:row.positionName||'',email:row.email||'',mobile:row.mobile||'',demoAccount:row.demoAccount,roleIds:row.roles.map((r:any)=>r.id),systems:[...row.systems],dataScopeType:row.dataScopeType||'ROLE',dataScopeValue:row.dataScopeValue||''});dialog.value=true}
async function save(){if(!form.username||!form.realName)return ElMessage.warning('请填写用户名和姓名');const creating=editingId.value===null;saving.value=true;try{const response=editingId.value?await http.put(`/admin/users/${editingId.value}`,form):await http.post('/admin/users',form);const id=editingId.value||response.data.data.id;await http.put(`/admin/users/${id}/data-scope`,{scopeType:form.dataScopeType,values:['DEPT','CUSTOM'].includes(form.dataScopeType)?String(form.dataScopeValue||'').split(',').map(x=>x.trim()).filter(Boolean):[]});ElMessage.success(editingId.value?'用户已更新':'用户已创建');dialog.value=false;if(creating)page.value=1;await load()}catch(e:any){ElMessage.error(e.message||'保存失败')}finally{saving.value=false}}
  ...
  <section class="admin-page">
    ...
    <el-dialog v-model="dialog" :title="editingId?'编辑用户':'新增用户'" width="760px" destroy-on-close>
      <el-form label-position="top"><el-row :gutter="16">
        ...
        <el-col :span="12"><el-form-item label="数据范围"><el-select v-model="form.dataScopeType" style="width:100%"><el-option label="沿用角色规则" value="ROLE"/><el-option label="全部数据" value="ALL"/><el-option label="本部门" value="DEPT"/><el-option label="仅本人" value="SELF"/><el-option label="无业务数据" value="NONE"/><el-option label="指定资源" value="CUSTOM"/></el-select></el-form-item></el-col><el-col v-if="['DEPT','CUSTOM'].includes(form.dataScopeType)" :span="24"><el-form-item label="范围值"><el-input v-model="form.dataScopeValue" :placeholder="form.dataScopeType==='DEPT'?'部门或车间编码，多个用逗号分隔；留空使用本人部门':'例如 CUSTOMER:C001,SALES_ORDER:SO001'" maxlength="500"/></el-form-item></el-col>
      </el-row></el-form>
