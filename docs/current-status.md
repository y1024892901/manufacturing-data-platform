...
- 计划 00 继续实施：266 个端点静态检查无鉴权注解缺口；已有实体未映射列 0，另有 123 张无实体表待逐表取舍，F4 严格验收未通过。
- 事件消费从「无消费者」推进到 QMS 收货事件真实消费；数据库烟测已验证 CONSUMED、重复业务事件只建一张检验单及回滚清理。
- MDM 四类主数据停用接口、悲观锁、前后状态审计与停用 Outbox 已实现，单元测试通过；完整 HTTP 生命周期仍待验收。
- 修复失败迁移 V40，已应用 V40 / V40.1 / V40.2；按用户要求不备份，执行范围停在 40.2。V41–V46 保留给计划 02。
- 22 项 Java 测试通过。01 仅部分缺陷落实，02–08 尚未完成，本轮不把十系统最低验收矩阵整体改成达标。

详细证据与待办见 [执行记录](plans/EXECUTION-2026-09-23.md)。以下为 2026-09-22 基线，涉及上述变更时以本节为准。
