...
public class EquipmentRepairController {
    ...
    @GetMapping public ApiResponse<Page<EquipmentRepair>> page(@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "20") int size) {
        return ApiResponse.ok(repairs.findAll(PageRequest.of(Math.max(0, page - 1), Math.min(200, size))));
    }

    @PostMapping public ApiResponse<EquipmentRepair> create(@RequestBody EquipmentRepair repair) {
    ...
    @PostMapping("/{id}/complete") public ApiResponse<EquipmentRepair> complete(@PathVariable Long id, @RequestParam String result) {
