...
新增四类主数据 `POST /api/mdm/{materials|products|boms|routings}/{id}/disable`，专用 DISABLE 权限（V40.2）；悲观锁、状态快照审计、停用事件共事务。单元测试验证四类停用与重复停用拒绝；完整创建至停用的 HTTP 验收未完成，五项总验收暂不升级。
