...
@RestController @RequestMapping("/api/mdm/boms") @RequiredArgsConstructor public class BomController{
 ...
 @GetMapping public ApiResponse<Page<Bom>> page(@RequestParam(required=false)String status,@RequestParam(defaultValue="1")int page,@RequestParam(defaultValue="20")int size){PageRequest p=PageRequest.of(Math.max(page-1,0),Math.min(size,200));return ApiResponse.ok(status==null||status.isBlank()?repo.findAll(p):repo.findByStatus(status,p));}
 @GetMapping("/consumable")public ApiResponse<List<Bom>> consumable(){return ApiResponse.ok(repo.findConsumable());}
 @GetMapping("/{id}")public ApiResponse<Bom> detail(@PathVariable Long id){return ApiResponse.ok(service.detail(id));}
 ...
 @PutMapping("/{id}") @PreAuthorize("hasAuthority('MDM:BOM:CHANGE')")public ApiResponse<Bom> update(@PathVariable Long id,@Valid @RequestBody BomCommand c){return ApiResponse.ok(service.update(id,c));}
 ...
 @PostMapping("/{bomId}/lines/{lineId}/substitutes") @PreAuthorize("hasAuthority('MDM:BOM:CHANGE')") public ApiResponse<com.mfg.mdm.entity.BomSubstitute> addSubstitute(@PathVariable Long bomId,@PathVariable Long lineId,@RequestBody SubstituteCommand c){return ApiResponse.ok(service.addSubstitute(bomId,lineId,c.materialCode(),c.priorityNo(),c.conversionRate()));}
 @DeleteMapping("/{bomId}/lines/{lineId}/substitutes/{id}") @PreAuthorize("hasAuthority('MDM:BOM:CHANGE')") public ApiResponse<Void> disableSubstitute(@PathVariable Long bomId,@PathVariable Long lineId,@PathVariable Long id){service.disableSubstitute(bomId,lineId,id);return ApiResponse.ok();}
