...
@RestController @RequestMapping("/api/plm/ecns") @RequiredArgsConstructor public class EngineeringChangeController{
 ...
 @GetMapping @PreAuthorize("hasAuthority('PLM:ECN:VIEW')") public ApiResponse<Page<EngineeringChange>> page(@RequestParam(defaultValue="1")int page,@RequestParam(defaultValue="20")int size){return ApiResponse.ok(repo.findAll(PageRequest.of(Math.max(page-1,0),Math.min(size,200))));}
 @GetMapping("/{id}") @PreAuthorize("hasAuthority('PLM:ECN:VIEW')") public ApiResponse<EngineeringChange> detail(@PathVariable Long id){return ApiResponse.ok(repo.findById(id).orElseThrow(()->new jakarta.persistence.EntityNotFoundException("ECN不存在")));}
